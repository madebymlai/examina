...
def detect_numbered_points(text: str) -> List[Dict[str, Any]]:
    ...
    for pattern, pattern_type in patterns:
        for match in re.finditer(pattern, text, re.MULTILINE | re.IGNORECASE):
            ...
            elif pattern_type in ['numeric_period', 'numeric_dash', 'italian_punto', 'italian_esercizio']:
                try:
...
def detect_transformation_keywords(text: str, language: str = 'en') -> List[Dict[str, Any]]:
    ...
    format_keywords = {
        'en': ['mealy', 'moore', 'dfa', 'nfa', 'pda', 'turing', 'fsm', 'automaton',
               'regular expression', 'regex', 'grammar', 'cfg', 'cnf', 'dnf',
               'state machine', 'circuit', 'boolean', 'truth table', 'karnaugh'],
        'it': ['mealy', 'moore', 'dfa', 'nfa', 'pda', 'turing', 'automa',
               'espressione regolare', 'regex', 'grammatica', 'circuito',
               'booleano', 'tabella di verità', 'karnaugh', 'mappa di karnaugh']
    }
    ...
    patterns = {
        'en': [
            ...
            (r'transform(?:ing)?\s+(?:the\s+)?(\w+(?:\s+\w+)?)\s+(?:to|into)\s+(?:a\s+|an\s+)?(\w+(?:\s+\w+)?)', 0.9),
            # "convert X to/into Y"
            (r'convert(?:ing)?\s+(?:the\s+)?(\w+(?:\s+\w+)?)\s+(?:to|into)\s+(?:a\s+|an\s+)?(\w+(?:\s+\w+)?)', 0.9),
            # "derive Y from X"
            (r'deriv(?:e|ing)\s+(?:a\s+|an\s+)?(\w+(?:\s+\w+)?)\s+from\s+(?:the\s+)?(\w+(?:\s+\w+)?)', 0.85),
            # "obtain Y from X"
            (r'obtain(?:ing)?\s+(?:a\s+|an\s+)?(\w+(?:\s+\w+)?)\s+from\s+(?:the\s+)?(\w+(?:\s+\w+)?)', 0.85),
        ...
        'it': [
            # "trasformare X in Y"
            (r'trasform(?:are|azione|ando|a)\s+(?:l\'|il\s+)?(?:automa\s+)?(?:di\s+)?(\w+)\s+in\s+(?:un\s+)?(?:automa\s+)?(?:di\s+)?(\w+)', 0.9),
            # "convertire X in Y"
            (r'convert(?:ire|endo|ito)\s+(?:l\'|il\s+)?(?:automa\s+)?(?:di\s+)?(\w+)\s+in\s+(?:un\s+)?(?:automa\s+)?(?:di\s+)?(\w+)', 0.9),
            # "ricavare Y da X"
            (r'ricav(?:are|ando|a)\s+(?:un\s+)?(?:automa\s+)?(?:di\s+)?(\w+)\s+dall?\'?(?:automa\s+)?(?:di\s+)?(\w+)', 0.85),
            # "ottenere Y partendo da X"
            (r'otten(?:ere|endo)\s+(?:un\s+|una\s+)?(\w+(?:\s+\w+)?)\s+partendo\s+da(?:l|ll\'|lla)?\s+(\w+(?:\s+\w+)?)', 0.85),
            # "Y equivalente a X"
            ...
            (r'(\w+(?:\s+\w+)?)\s*(?:→|->)\s*(\w+(?:\s+\w+)?)', 0.95),
            # "trasformazione da X a Y"
            (r'trasformazione\s+da(?:l|ll\'|lla)?\s+(\w+(?:\s+\w+)?)\s+a(?:l|ll\'|lla)?\s+(\w+(?:\s+\w+)?)', 0.9),
        ]
    }
...
def classify_procedure_type(point_text: str, language: str = 'en') -> str:
    ...
    keywords = {
        'en': {
            'transformation': [
                'transform', 'convert', 'derive', 'equivalent', 'obtain',
                '→', '->'
            ],
            'design': [
                'design', 'construct', 'build', 'create', 'develop',
                'draw', 'sketch', 'implement from scratch'
            ],
            'verification': [
                'verify', 'prove', 'check', 'validate', 'demonstrate',
                'show that', 'confirm', 'test whether'
            ],
            'minimization': [
                'minimize', 'optimize', 'reduce', 'simplify',
                'implication table', 'state reduction', 'equivalent states'
            ],
            'analysis': [
                'analyze', 'calculate', 'determine', 'find',
                'compute', 'identify', 'evaluate', 'count'
            ],
            'implementation': [
                'implement', 'code', 'program', 'circuit',
                'logic gates', 'truth table', 'hardware'
            ]
        },
        'it': {
            'transformation': [
                'trasform', 'convert', 'ricav', 'equivalente', 'otten',
                '→', '->'
            ],
            'design': [
                'disegn', 'costrui', 'crea', 'svilupp',
                'progett', 'realizza', 'schema'
            ],
            'verification': [
                'verific', 'dimostr', 'controlla', 'valid',
                'mostra che', 'conferma', 'testa se'
            ],
            'minimization': [
                'minimizza', 'ottimizza', 'riduci', 'semplifica',
                'tabella delle implicazioni', 'riduzione degli stati', 'stati equivalenti'
            ],
            'analysis': [
                'analizza', 'calcola', 'determina', 'trova',
                'individua', 'valuta', 'conta'
            ],
            'implementation': [
                'implementa', 'codifica', 'programma', 'circuito',
                'porte logiche', 'tabella di verità', 'hardware'
            ]
        }
    }

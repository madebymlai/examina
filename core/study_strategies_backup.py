...
class StudyStrategyManager:
    ...
    Supports both hardcoded strategies (high quality, fast) and LLM-generated
    strategies (automatic, scalable) with caching.
    """

    def __init__(self, language: str = "en", enable_llm_fallback: bool = None):
        """
        Initialize the study strategy manager.

        Args:
            language: Output language ("en" or "it")
            enable_llm_fallback: Enable LLM for unmapped strategies (default: from config)
        """
        self.language = language

        # Initialize LLM fallback based on config
        if enable_llm_fallback is None:
            from config import Config
            enable_llm_fallback = Config.STUDY_STRATEGY_LLM_FALLBACK

        self.enable_llm_fallback = enable_llm_fallback
        self.llm_manager = None

        # Lazy load LLM manager only if fallback enabled
        if self.enable_llm_fallback:
            try:
                from models.llm_manager import LLMManager
                self.llm_manager = LLMManager()
            except Exception as e:
                print(f"[WARNING] Could not initialize LLM manager for strategy fallback: {e}")
    ...
    def get_strategy_for_core_loop(self, core_loop_name: str, difficulty: str = "medium") -> Optional[Dict]:
        ...
        Tries hardcoded strategies first (fast, high quality), then falls back
        to LLM-generated strategies if enabled (automatic, scalable).
        ...
        """
        # Try hardcoded strategies first (fast path)
        ...
        # Fallback to LLM-generated strategy
        if self.enable_llm_fallback and self.llm_manager:
            print(f"[INFO] No hardcoded strategy for '{core_loop_name}', generating with LLM...")
    ...
    # ========================================================================
    # LLM Fallback Methods (for unmapped core loops)
    # ========================================================================
    ...
    def _generate_strategy_with_llm(self, core_loop_name: str, difficulty: str) -> Optional[Dict]:
        ...
        if cached:
            print(f"[INFO] Using cached LLM strategy for '{core_loop_name}'")
            return cached
        ...
        try:
            ...
            if strategy:
                ...
                print(f"[INFO] Generated and cached LLM strategy for '{core_loop_name}'")
                return strategy
            else:
                print(f"[WARNING] Failed to parse LLM strategy for '{core_loop_name}'")
                return None

        except Exception as e:
            print(f"[ERROR] LLM strategy generation failed for '{core_loop_name}': {e}")
            return None
...
Return ONLY the JSON, no explanation or markdown formatting."""
    ...
    def _parse_llm_strategy(self, llm_response: str, core_loop_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            print(f"[DEBUG] Response was: {llm_response[:500]}...")
            return None
        except Exception as e:
            print(f"[ERROR] Unexpected error parsing LLM strategy: {e}")
            return None
